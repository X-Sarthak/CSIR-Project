...
function BookingForm({ meeting, setShowBookingForm }: BookingFormProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(
        `/user/booking/schedule/send/${meeting.meeting_id}`,
        {
          meeting_title: title,
      ...
      navigate("/login/user/request/display");
    } catch (error: any) {
  ...
  return (
    <div className="relative w-full h-full overflow-y-auto flex justify-center items-center bg-gray-800 bg-opacity-50">
      ...
      >
        <h2 className="text-xl font-serif mb-4">Booking Form</h2>
