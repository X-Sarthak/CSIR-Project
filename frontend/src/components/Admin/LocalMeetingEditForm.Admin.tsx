...
function LocalMeetingEdit() {
  ...
  const [formDataVisible, setFormDataVisible] = useState(false); // New state for form visibility
  ...
  const fetchFormData = async (meetingId: string): Promise<void> => {
    ...
    try {
      ...
      if (data && Object.keys(data).length > 0) {
        ...
        setFormDataVisible(true); // Show the form once data is fetched
        toast.success("Fetch Successfully");
      } else {
        setFormDataVisible(false); // Hide the form if no data is found
        toast.error("No data found for the selected Meeting ID.");
      }
    } catch (error) {
      // Check if the error is due to a 404 status
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        setFormDataVisible(false); // Hide the form if Meeting ID is not found
        toast.error("Meeting ID not found.");
      } else {
        console.error("Error fetching form data:", error);
        toast.error("Error fetching form data.");
        setFormDataVisible(false); // Hide the form on error
      }
    }
  };
  ...
  const handleSearch = () => {
    if (meetingId) {
      ...
      setFormDataVisible(false); // Hide form if no meeting ID is provided
                ...
                <div>
                  ...
                  <button
                    ...
                    className={`bg-blue-500 hover:bg-blue-600 text-white font-serif px-12 py-2 mt-2 ml-2 rounded-md transition duration-300 ease-in-out ${
                      !meetingId ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                    disabled={!meetingId}
                  >
                    Search
